alter table public.pixel_battle_profiles
  drop constraint if exists pixel_battle_profiles_balance_check;

alter table public.pixel_battle_profiles
  add constraint pixel_battle_profiles_balance_check
  check (balance between 0 and 200);
...
begin
  ...
  new_balance := least(200, current_profile.balance + generated);

  if generated > 0 and new_balance <> current_profile.balance then
    update public.pixel_battle_profiles
    set balance = new_balance,
        last_recharge_at = case
          when new_balance = 200 then now()
