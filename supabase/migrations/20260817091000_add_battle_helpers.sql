...
begin
  ...
  is_helper := lower(current_profile.email) in (
    'goodheatocean@gmail.com',
    'tzhutimz@gmail.com',
    'ratmirtyutyunik2012@gmail.com'
  );
  has_power_tools := is_owner or is_helper;
  safe_size := case
    when is_helper and erase_pixels then 1
    when has_power_tools then least(greatest(brush_size, 1), 4)
    else 1
  end;

  if erase_pixels and not has_power_tools then
  ...
  if not has_power_tools and affected_count > 0 then
  ...
  next_balance := case
    when has_power_tools or affected_count = 0 then current_profile.balance
